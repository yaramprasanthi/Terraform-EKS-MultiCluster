            ...
            steps {
                script {
                    ...
                    if (status != 'NOT_FOUND' && params.DESTROY_CONFIRMATION == 'yes') {
                        ...
                        // Step 1: Configure kubeconfig (ignore errors if cluster endpoint is gone)
                        sh "aws eks update-kubeconfig --name ${env.CLUSTER_NAME} --region ${env.AWS_REGION} || echo 'Cluster endpoint not reachable, continuing...'"
        
                        // Step 2: Delete all LoadBalancer Services
                        sh """
                        ...
                        // Step 4: Destroy Terraform-managed resources
                        ...
                        sh "aws eks delete-cluster --name ${env.CLUSTER_NAME} --region ${env.AWS_REGION} || echo 'Cluster already deleted'"
                        echo "✅ Cluster and all dependent resources destroyed successfully. Exiting pipeline."
